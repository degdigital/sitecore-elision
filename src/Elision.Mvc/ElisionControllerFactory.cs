...
namespace Elision.Mvc
{
    public class ElisionControllerFactory : DefaultControllerFactory
    {
        ...
        protected virtual IController CreateControllerUsingConfigurationFactory(string controllerName)
        {
            ...
            try
            {
                node = _knownTypes.ContainsKey(controllerName)
                           ? _knownTypes[controllerName]
                           : Factory.GetConfigNode(controllerName);
            }
            catch
            {
                node = null;
            ...
            Log.Debug(string.Format("Couldn't find {0} controller in Web.config", controllerName), GetType());
